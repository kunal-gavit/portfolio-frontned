...
export const ResumeModal = ({ isOpen, onClose }) => {
  ...
  const handleDownload = () => {
    ...
    analytics.trackResumeDownload('resume_modal');
    if (resumeUrl.startsWith('http')) {
  ...
  const handleViewPdf = () => {
    if (!pdfAvailable) return;
    analytics.trackResumeView('resume_modal_view_button');
    window.open(resumeUrl, '_blank', 'noopener,noreferrer');
  };

  const handlePrint = () => {
    trackEvent('resume_print', { timestamp: new Date().toISOString() });
    window.print();
  };
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/85 backdrop-blur-md overflow-y-auto print:p-0 print:bg-white">
        <motion.div
          ...
          className="w-full max-w-4xl max-h-[92vh] bg-zinc-900 border border-zinc-700/80 rounded-3xl shadow-2xl overflow-y-auto text-zinc-100 flex flex-col my-auto print:max-h-none print:border-none print:shadow-none print:bg-white print:text-black print:rounded-none"
        >
          {/* Header Controls Bar (Hidden during print) */}
          <div className="sticky top-0 z-20 flex flex-wrap items-center justify-between gap-3 px-6 py-4 bg-zinc-950/95 backdrop-blur-md border-b border-zinc-800 print:hidden">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="font-heading font-bold text-base text-zinc-100">
                  Resume Center — {profile.name}
                </h3>
                <span className="text-[11px] font-mono text-zinc-400">
                  ATS-Optimized • Engineering & Developer Edition {new Date().getFullYear()}
                </span>
              </div>
            </div>

            {/* Action Buttons: Download, View, Print */}
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrint}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold transition cursor-pointer"
                title="Print Resume (A4 Optimized)"
              >
              ...
              <button
                ...
                title={pdfAvailable ? 'Open PDF in new tab' : 'PDF not found'}
              >
              ...
              <button
                ...
                disabled={!pdfAvailable}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition shadow-md cursor-pointer ${
                  pdfAvailable
                    ? 'bg-cyan-500 hover:bg-cyan-400 text-zinc-950 shadow-cyan-500/20'
                    : 'bg-zinc-800 text-zinc-500 cursor-not-allowed shadow-none'
                }`}
                title={pdfAvailable ? `Download ${resumeFilename}` : 'PDF not found'}
              >
              ...
              <button
                onClick={onClose}
                className="p-1.5 rounded-lg text-zinc-400 hover:text-white hover:bg-zinc-800 transition ml-1 cursor-pointer"
          ...
          {/* Fallback Notice if PDF is unavailable */}
          {!pdfAvailable && !isCheckingPdf && (
            <div className="mx-6 mt-4 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-center gap-2 print:hidden">
              <AlertCircle className="w-4 h-4 shrink-0 text-amber-400" />
              <span>
                <strong>Resume PDF unavailable:</strong> PDF file can be placed at{' '}
                <code className="bg-amber-500/20 px-1.5 py-0.5 rounded font-mono">
                  public/resume/Kunal_Gavit_Resume.pdf
                </code>
                . In the meantime, you can review or print the live interactive resume below!
              </span>
            </div>
          )}

          {/* Printable / Renderable Resume Document (A4 format) */}
          <div className="p-8 sm:p-12 space-y-8 bg-zinc-900 text-zinc-200 text-xs leading-relaxed max-w-3xl mx-auto w-full print:p-4 print:text-black print:bg-white print:max-w-full">
            {/* Header / Identity */}
